...
if(isset($_GET['token']) && !empty($_GET['token'])&& isset($_GET['ge']) && !empty($_GET['ge'])){
    ...
    if(count($user)>0){
         if($_GET['ge']=="position" && !isset($_GET['id']) && empty($_GET['id'])){
                //on prepare une requette sql
                $requ=$pdo->prepare("select ideolienne,Latitude,Longitude from eolienne");
                //on execute la requette sql
                $requ->execute();
                //on recupere les donnee dans une liste
                $lt=array();
                while($row=$requ->fetch()){
                    $lt[$row['ideolienne']] = array('Latitude'=>$row['Latitude'],'Longitude'=>$row['Longitude']);
                }
                //on converti la liste lt en json
                header('Content-Type: application/json');
                echo json_encode($lt, JSON_PRETTY_PRINT);
            }
            else if($_GET['ge']=="position" && isset($_GET['ge']) && !empty($_GET['ge'])){
                //on prepare une requette sql
                $requ=$pdo->prepare("select ideolienne,Latitude,Longitude from eolienne where ideolienne like '".$_GET['id']."%'" );
                //on execute la requette sql
                $requ->execute();
                //on recupere les donnee dans une liste
